...
public class PGVRuleFactory {
    ...
    public static final String VALIDATE_RULES_NAME = "buf.validate.field";
    ...
    public List<OptionElement> getValidationRule(XSParticle parentParticle) {
        ...
        if (configuration.includeValidationRules) { // TODO SKIP FOR NOW AS RULES ARE NOT COMPILING
            ...
            if (minOccurs == 1 && maxOccurs == 1) {
                validationRules.add(new OptionElement("(" + VALIDATE_RULES_NAME + ").required", OptionElement.Kind.BOOLEAN, true, false));
                ...
                validationRules.add(new OptionElement("(" + VALIDATE_RULES_NAME + ").repeated", OptionElement.Kind.MAP, minMaxParams, false));
            }
    ...
    public List<OptionElement> getValidationRule(XSAttributeDecl attributeDecl) {
        ...
        if (configuration.includeValidationRules) {
            return getValidationRule(attributeDecl.getType());
        }
    ...
    public List<OptionElement> getValidationRule(XSSimpleType simpleType) {
        ...
        if (configuration.includeValidationRules) {
            ...
            if (typeName != null && basicTypes.contains(typeName)) {
                ...
                } else if ("token".equals(baseType)) {
                    addOptionForEnumeration(validationRules, declaredFacets);
                }
    ...
    private void addOptionForEnumeration(List<OptionElement> validationRules, Collection<? extends XSFacet> declaredFacets) {
        Map<String, Object> parameters = new HashMap<>();
        for (XSFacet facet : declaredFacets) {
            switch (facet.getName()) {
